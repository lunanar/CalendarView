...
#define DEFAULT_CELL_WIDTH 15
#define CELL_BORDER_WIDTH 0
#define DEFAULT_CELL_HEIGHT ScreenWidth/7
...
@property (nonatomic,assign) CGFloat cellHeight;   //日历cell的高度
@property (nonatomic,strong) UILabel *label;
@property (nonatomic,strong) NSMutableArray *labelsArr;
@property (nonatomic,strong) NSMutableArray *imgArr;   //选中的图片
...
//    //[highlight setBackgroundColor:[UIColor greenColor]];
...
//    self.highlight = highlight;
    ...
    [prevButton setImage:[UIImage imageNamed:@"order_left"] forState:UIControlStateNormal];
    ...
    [nextButton setImage:[UIImage imageNamed:@"order_right"] forState:UIControlStateNormal];
...
//    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, TOP_HEIGHT, ScreenWidth, 1)];
//    [line setBackgroundColor:[UIColor colorFromHexCode:@"595757"]];
...
//    [daysHeader setBackgroundColor:[UIColor greenColor]];
...
//        dayOfWeekLabel.shadowOffset = CGSizeMake(0, 1);
    ...
    for (NSInteger i = 1; i <= 42; i++) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, ScreenWidth/14+5, ScreenWidth/7, ScreenWidth/14-5)];
        label.text = @"";
        label.font = [UIFont systemFontOfSize:10];
        label.textColor = [UIColor redColor];
        label.textAlignment = NSTextAlignmentCenter;
        [dateButton addSubview:label];
        [self.labelsArr addObject:label];
        
        //不减4的话会有重叠的部分 不知道为啥
        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(2, 0, ScreenWidth/7-4, DEFAULT_CELL_HEIGHT)];
        [imgView setImage:[UIImage imageNamed:@"choice"]];
//        imgView.hidden = NO;
...
- (void)layoutSubviews {
    ...
    CGFloat containerWidth = ScreenWidth - (CALENDAR_MARGIN * 2);
    self.cellWidth = (floorf(containerWidth / 7.0));
    NSLog(@"width:%f",self.cellWidth);
    ...
    //CGFloat containerHeight = (numberOfWeeksToShow * (self.width + CELL_BORDER_WIDTH) + DAYS_HEADER_HEIGHT);
    CGFloat containerHeight = (numberOfWeeksToShow * self.cellHeight +DAYS_HEADER_HEIGHT);
    if ([self.delegate respondsToSelector:@selector(calendarHeight:)]) {
        CGFloat height = containerHeight + TOP_HEIGHT;
        
        //[self.delegate calendarHeight:0];
        [self.delegate calendarHeight:height];
    }
    ...
    self.titleLabel.frame = CGRectMake(0, 0, self.bounds.size.width, TOP_HEIGHT-1);
    self.titleLabel.textAlignment = NSTextAlignmentCenter;

    //self.prevButton.frame = CGRectMake(BUTTON_MARGIN, BUTTON_MARGIN, 30, 28);
    self.prevButton.frame = CGRectMake(ScreenWidth/2 -100-15, BUTTON_MARGIN, 12, 15);
    //self.nextButton.frame = CGRectMake(self.bounds.size.width - 48 - BUTTON_MARGIN, BUTTON_MARGIN, 48, 38);
    self.nextButton.frame = CGRectMake(ScreenWidth/2 + 100, BUTTON_MARGIN, 12, 15);

    self.calendarContainer.frame = CGRectMake(CALENDAR_MARGIN, CGRectGetMaxY(self.titleLabel.frame), containerWidth, containerHeight); //containerHeight
    self.daysHeader.frame = CGRectMake(0, 0, self.calendarContainer.frame.size.width, DAYS_HEADER_HEIGHT);

    CGRect lastDayFrame = CGRectZero;
    for (UILabel *dayLabel in self.dayOfWeekLabels) {
        dayLabel.frame = CGRectMake(CGRectGetMaxX(lastDayFrame) + CELL_BORDER_WIDTH, lastDayFrame.origin.y, self.cellWidth, self.daysHeader.frame.size.height);   //self.daysHeader.frame.size.height
    ...
    while ([date laterDate:endDate] != date) {
        ...
        UIImageView *imgView = [self.imgArr objectAtIndex:dateButtonPosition];
//        imgView.hidden = YES;
        
        CKDateItem *item = [[CKDateItem alloc] init];
        if ([self _dateIsToday:dateButton.date]) {
            item.textColor = [UIColor colorFromHexCode:@"39aad5"];
        ...
        UILabel *label = [self.labelsArr objectAtIndex:dateButtonPosition];
        label.text = @"余下";
        if ([self.remainStockDic objectForKey:dateStr] != nil) {
            label.text = [@"余" stringByAppendingString:[self.remainStockDic objectForKey:dateStr]];
        }
...
//            title = @"六";
//        }
        [[self.dayOfWeekLabels objectAtIndex:i] setText:[[day substringFromIndex:1] uppercaseString]];
        i++;
    }
}
...
- (NSMutableArray *)labelsArr {
    if (_labelsArr == nil) {
        _labelsArr = [[NSMutableArray alloc] initWithCapacity:10];
    }
    return _labelsArr;
}
...
- (void)_setDefaultStyle {
    ...
    [self setTitleColor:[UIColor colorFromHexCode:@"595757"]];
    [self setTitleFont:[UIFont boldSystemFontOfSize:14.0]];

    [self setDayOfWeekFont:[UIFont boldSystemFontOfSize:12.0]];
    //[self setDayOfWeekBottomColor:[UIColor yellowColor] topColor:[UIColor whiteColor]];
...
- (CGRect)_calculateDayCellFrame:(NSDate *)date {
    ...
    return CGRectMake(placeInWeek * (self.cellWidth + CELL_BORDER_WIDTH), (row * (self.cellHeight + CELL_BORDER_WIDTH)) + CGRectGetMaxY(self.daysHeader.frame) + CELL_BORDER_WIDTH, self.cellWidth, self.cellHeight); //self.cellwidth
}
...
- (void)setMonthButtonColor:(UIColor *)color {
    [self.prevButton setImage:[JYCalendarView _imageNamed:@"order_left" withColor:color] forState:UIControlStateNormal];
    [self.nextButton setImage:[JYCalendarView _imageNamed:@"order_right" withColor:color] forState:UIControlStateNormal];
}
